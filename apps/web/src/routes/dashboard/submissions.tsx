...
function SubmissionsPage() {
  ...
  const salon = salons.find((s) => s.status === "open");
    ...
    return (
      <div className="p-8 max-w-2xl">
        ...
        <div className="rounded-lg border border-border p-6">
          <p className="text-sm text-muted-foreground">No salon is currently open for submissions.</p>
        </div>
      </div>
    );
  }

  return <ActiveSalonSubmissions salon={salon} />;
}
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        {salon.name} · {MONTH_NAMES[salon.month]} {salon.year}
      </p>

      <div className="mb-6">
        <p className="text-xs text-muted-foreground">
          {totalActive} / {salon.maxSubmissionsPerMember} salon submissions used
        </p>
      </div>
      ...
      {/* Submit form */}
      {activeCount < maxPerCategory && totalActive < salon.maxSubmissionsPerMember ? (
        <div className="mb-8">
        ...
        <div className="mb-8 rounded-lg border border-border p-4">
          <p className="text-sm text-muted-foreground">
            Maximum submissions reached for this {activeCount >= maxPerCategory ? "category" : "salon"}.
          </p>
        </div>
      )}
